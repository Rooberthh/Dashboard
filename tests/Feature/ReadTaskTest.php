<?php
    ...
    class ReadTaskTest extends TestCase
    {
        ...
        function a_user_can_fetch_tasks ()
        {
            ...
            create('App\Models\Task', ['status_id' => $status->id]);
        ...
        {
            $status = create('App\Models\Status');
            create('App\Models\Task', ['status_id' => $status->id, 'order' => 10]);
            create('App\Models\Task', ['status_id' => $status->id, 'order' => 9]);

            $response = $this->json('get', "api/statuses/$status->id/tasks")->getContent();
